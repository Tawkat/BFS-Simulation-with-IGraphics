...
int Q[1000]={0};
int xn[1000],flag1=0,src,yp[1000]={0},ax[1000],ay[1000],bx[1000],by[1000],desx[1000],desy[1000],fx,fy,final,w2;//,yn[1000]={0};
int cx,cy,cz;
...
double dx,dy;//dy=my;
int i1=1,flag3=1,flag4=1,flag5=1,i2=1,count=0;
...
void bfs(int n,int s)
{
    ...
    do
    {
		...
		desy[0]=fy;
		int check=final;
		for(w2=1;desx[w2]!=mX[src] || desy[w2]!=mY[src];w2++)
		{
			desx[w2]=mX[parent[check]];
			desy[w2]=mY[parent[check]];
			if(parent[check]==src)
			{
				break;
			}////
...
void iDraw()
{
	...
	desx[1]=mX[final];
		desy[1]=mY[final];
		int check=final;
		for(w2=2;desx[w2]!=mX[src] || desy[w2]!=mY[src];w2++)
		{
			desx[w2]=mX[parent[check]];
			desy[w2]=mY[parent[check]];
			if(parent[check]==src)
	...
	else if(flag5==0)
	{
		...
		if(flag4==1)
	{
		mx=desx[w2],my=desy[w2],nx=desx[w2-1],ny=desy[w2-1];
			x1=mx,k1=my,x2=nx,y2=ny,p,c=(ny-((ny-my)/(nx-mx+0.00))*nx)+0.00;
			/*if(mx>nx)
			{
				c=c*(-1);
