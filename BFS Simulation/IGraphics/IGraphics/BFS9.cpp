...
int mouseX, mouseY,node=0,a=0,i=0,j=0,z,y,switch_node=0,edge_node=0,source_node=0,process=0,bfs_path=0,flag_bfs=0,mX[50],mY[50],eX[50],eY[50],c[50],sx,sy,edge,vertex[50],parent[100],v1,v2,flag=0;
char clear=0;
...
void iDraw()
{
	...
	if(process && source_node)
	{
        ...
        for(int i=1;i<=node;i++)
        {
			...
			cx=30+50*parent[i];
			cy=250-25*parent[i]*i*(-1);
			cz=100+30*parent[i]*i*(-1);
			if(cx>255)
			{
				cx=255+20*parent[i]*i*(-1);
			}
			if(cx<0)
			{
				cx=255-10*parent[i];
